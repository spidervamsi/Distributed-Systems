...
public class SimpleDynamoProvider extends ContentProvider {
		...
		try {
			...
			if(selection.equals("@")){
				...
				if(!context.deleteFile(filename)){
					try {
						int port = continueOrNot(selection);
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		try {
			int targetPort = continueOrNot(values.get("key").toString());
			Log.i("targetPort key",values.get("key").toString());
			Log.i("targetPort "+values.get("key").toString(),Integer.toString(targetPort));
...
//				int targetPort = findMatch(values.get("key").toString());
			...
			Log.i("targetPort passed",values.get("key").toString());
...
//        [11120, 11116, 11108, 11112, 11124] Sorted array
}catch(Exception e){}
...
//            s1<s2 -> -2
//            s1>s2 -> +2
	private int continueOrNot(String msg){
		...
		try{
			...
			for (int i=1;i<portsSortedList.size();i++){
				...
				if(genHash(msg).compareTo(genHash(Integer.toString(portsSortedList.get(i)/2))) <= 0 && genHash(msg).compareToIgnoreCase(genHash(Integer.toString(portsSortedList.get(i-1)/2))) > 0){
		...
		protected String doInBackground(String... msgs) {
			...
			if(msgs[0].contains("connect")){
				try {
					...
					Log.i("Exception Connectclient","Exception"+e.getMessage());
					e.printStackTrace();
				}
				...
				try {
					...
					Log.i("Exception Client","Exception "+e.getMessage());
